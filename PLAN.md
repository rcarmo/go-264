...
- B-slice Direct-mode reconstruction is still a bounded fallback rather than FFmpeg-equivalent colocated temporal/spatial derivation. Full `B_Direct_16x16` currently seeds spatial fallback MVs and per-8×8 representatives, but FFmpeg can internally derive 8×8/direct sub-shapes from the colocated future MB even when the syntax is not `B_8x8`. Direct sub-MB modes now seed/write back spatial fallback motion for neighbour parity, saved reference frames carry list0 4×4 motion/ref caches, the B_8x8 zero helper follows FFmpeg's `x8*3/y8*3` representative rule, and `compare_direct_writeback.py` proves direct representatives and motion-cache write-back agree in the focused POC=6 gate (`compared=1111 diffs=0`). Remaining work is true FFmpeg-equivalent Direct-mode reference/MV derivation: the current focused direct mismatches are around `mb=0291`, `mb=0297`, `mb=0307`, `mb=0331`, and `mb=0339`.
- Multi-frame CABAC diagnostics are now usable for post-motion B parity via `scripts/b_bidi_trace.sh`/`scripts/compare_bidi_trace.py`. After the B cache layer, direct write-back diagnostics, and P16x8 top-before-bottom MVP fix, the reference B frame through `MB_LIMIT=400` is down to one post-motion diff at `mb=0371` (`compared=136 diffs=1`). `scripts/b_direct_trace.sh` also emits `FFCOLZERO/FFCOLZERO8`, `GOCOLZERO`, `GOMOTSAVE`, `GOMOTWRITE`, and a `gowrite.diff` artifact so write-back regressions are caught separately from real FFmpeg-vs-Go Direct-mode derivation mismatches.
