...
| `bbb-frame0` CABAC | 12.39 dB avg PSNR |
...
Current first-divergence status: `testsrc_cabac_p.h264` and `bbb_annexb.h264` first-frame MB syntax summaries now report `NO_DIVERGENCE in compared fields`. The active mismatch class has moved to reconstruction parity. Recent source-grounded fixes promoted FFmpeg-style CABAC intra edge contexts, High-profile intra `transform_size_8x8_flag` consumption, luma Intra_8x8 filtered DC references, the top-edge Intra_8x8 `LEFT_DC_PRED` reconstruction case, FFmpeg chroma DC quadrant/edge prediction, FFmpeg `pred_intra_mode` unavailable-neighbour handling, separate I4x4-derived right/bottom mode caches for CABAC I8x8 neighbour prediction, and aligned-buffer reconstruction for partial edge macroblocks. `GO264_RECON_TRACE=1` plus local FFmpeg instrumentation is the current tool path for isolating remaining prediction/residual/output mismatches. Raw CABAC 8×8 residual `levelseq` traces now match FFmpeg for the compared prefix, and `ff_raw_coeff` confirms Go raw coefficients convert exactly to FFmpeg `rawlevels` for the first residual-focused block. The active gap is therefore after raw coefficient placement: dequant/IDCT scaling and prediction class interactions. Blanket FFmpeg storage/qmul, ¼ dequant scaling, residual-handoff transpose, and FFmpeg IDCT-rounding candidates were tested and rejected because they improve one block or no block while regressing the hard `bbb` gate.
...
- MV/ref caches are the single source of motion-prediction context in the decoder and trace tooling. Cache reads/fills and CABAC MV/ref context helpers reject malformed strides, short slices, and negative origins instead of panicking in direct helper/tool use. CABAC MVD context cache write-back mirrors FFmpeg by keeping the full signed MVD for reconstruction while storing `min(abs(mvd),70)` for future context selection.
