...
func (c bMotionCache) decodeCABACBidiMB(dec *cabac.CABACDecoder, models []cabac.CABACCtx,
	...
	return decodeCABACBidiMB(dec, models,
		...
		directColocated, directL0Frames, directColPOC,
		transform8x8Mode, transform8x8Ctx,
		leftMBType, topMBType,
		leftChromaPred, topChromaPred,
		leftEdge8x8, topEdge8x8)
...
func (c bMotionCache) saveL0ToFrame(f *frame.Frame, mbFFTypes []uint32, l0Frames []*frame.Frame) {
	...
	// A single slice's temporal list has the same order as its original List 0.
	f.TemporalRefIdxL0 = append(f.TemporalRefIdxL0[:0], c.ref[0]...)
