...
`TestAudioImportBoundary` checks all runtime Go imports, including inactive architecture files. Run focused tests with `CGO_ENABLED=0 go test ./audio/...`; these do not require video fixtures. `.github/workflows/audio.yml` runs default and `purego` tests, vet, the full race detector and synthetic FFmpeg oracle matrix on amd64, plus default, `purego`, vet and race tests on a native Linux ARM64 runner. Race builds skip two exact-bit tests because instrumentation changes Go floating-point code generation while assembly is uninstrumented; default native tests retain exact NEON parity and the pinned production synthesis digest. Cross-builds and QEMU remain functional checks, not native architecture evidence.
...
Four-hour sparse-WAV and actual SIGKILL checkpoint stages pass along with scalar/assembly, malformed-input and replay tests. The full `audio/...` race suite passes on linux/amd64 in Go 1.26.2. Native ARM64 race coverage executes every audio test except the two instrumentation-sensitive exact-bit checks described above. Synthetic two-second canonical decode measures about 138× realtime and isolated resampling about 750×; these exceed the provisional 50×/100× targets on those fixtures only. Generalisation to other encoders, tools, long workloads and physical power-loss behaviour has not been established. No tag or serving-default switch has been made.
