...
The new `audio/` tree is explicitly licensed under [MIT](LICENSE); imported notices remain applicable. The inspected repository has no root licence file, so this statement does not relicense or certify the pre-existing video tree. Earlier reports describing the entire module as licence-audited MIT were too broad. Audio-only imports do not depend on those video packages.
...
Four-hour sparse-WAV and actual SIGKILL checkpoint stages pass along with focused race, scalar/assembly, malformed-input and replay tests. Synthetic two-second canonical decode measures about138× realtime and isolated resampling about750×; these exceed the provisional50×/100× targets on those fixtures only. Generalisation to other encoders/tools/long workloads and physical power-loss guarantees is not established. Optional consumer09d2db2 is committed separately; active-branch merge and remote publication are pending. No tag or serving-default switch has been made.
