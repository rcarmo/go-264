...
`TestAudioImportBoundary` checks all runtime Go imports, including inactive architecture files. Run focused tests with `CGO_ENABLED=0 go test ./audio/...`; these do not require video fixtures. `.github/workflows/audio.yml` runs default and `purego` tests, vet, all audio packages under the race detector, the synthetic FFmpeg oracle matrix on amd64, and the default, `purego` and race suites on a native Linux ARM64 runner. Cross-builds and QEMU remain useful functional checks, but only that hosted ARM64 job counts as native architecture evidence.
